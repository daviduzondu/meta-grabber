    ...
    try {
        const response = await fetch(target)
            ...
            $('meta').each(function(index){
                if($(this).attr('property')==="og:image" && !metadata.img) {metadata.img = $(this)['0'].attribs.content; metadata.error=false; metadata.message+="image_"}

                if($(this).attr('name')?.includes("image") && !metadata.img) {metadata.img = $(this)['0'].attribs.content; metadata.error=false; metadata.message+="image_"}
                ...
                if($(this).attr('name')?.includes("description") && !metadata.description){ metadata.description = $(this)['0'].attribs.content; metadata.error=false; metadata.message="description_"}
